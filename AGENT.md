...
Types:
17. `FLOAT` for physical quantities end-to-end (FC struct fields, wire
    ...
    conversion, so no FLOAT deviation is needed.
...
- Rule 3 applies to control-flow braces; function closing braces get tagged:
  `} // function_name`.
- `FLOAT` internals apply per the unit-mapping note above — the wire/persistence
