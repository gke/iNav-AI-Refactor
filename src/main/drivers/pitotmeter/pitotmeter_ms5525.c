...
static bool ms5525_start(pitotDev_t * pitot) {
    ...
    if (step == 0) return busWrite(pitot->busDev, CMD_ADC_CONV + CMD_ADC_D1 + CMD_ADC_4096, 1);
} // ms5525_start
...
static bool ms5525_read(pitotDev_t * pitot) {
    ...
    uint32_t adc_val = 0;
    if (!ms5525_read_adc(pitot, &adc_val)) return false;;
...
static void ms5525_calculate(pitotDev_t * pitot, float *pressure, float *temperature) {
    ...
    if (up != 0 && ut != 0) {
        ...
        if (pressure) *pressure = (float)p_raw * 0.6894757f;;

        if (temperature) *temperature = C_TO_KELVIN(temp / 100.0f);;
    }
} // ms5525_calculate
...
bool ms5525Detect(pitotDev_t * pitot) {
    ...
    if (pitot->busDev == NULL) return false;;
    ...
    for (int i = 0; i < 6; i++) ctx->c[i] = prom[i+1];;
