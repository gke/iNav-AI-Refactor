...
void pgResetFn_controlProfiles(controlConfig_t *instance) {
    for (int i = 0; i < MAX_CONTROL_PROFILE_COUNT; i++) RESET_CONFIG(controlConfig_t, &instance[i],
    .throttle = {
...
} // pgResetFn_controlProfiles

void setControlProfile(uint8_t profileIndex) {
    if (profileIndex >= MAX_CONTROL_PROFILE_COUNT) profileIndex = 0;;
